...
function resetPageColor(){
        ...
        threshold = Array(8).fill(map(130, 0, 768, 0, 255));
}
if(!checkmessage){
    distanceValue.style.color = "#CCCCCC";
    textangle.style.color = "#CCCCCC";
...
let threshold = Array(8).fill(map(130, 0, 768, 0, 255));
...
function compareThreshold(index) {
    ...
    threshold[index] = parseFloat((Math.min(threshold[index], irValue * 1.5))).toFixed(1);
